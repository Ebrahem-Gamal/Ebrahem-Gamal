# Ebrahem Gamal
### Network & DevOps Engineer | Electrical & Communications Engineer

I am an Electrical and Communications Engineering student at HTI with a GPA of 3.41. I specialize in Network Engineering and DevOps, focusing on automating infrastructures and building reliable embedded systems.

---

## Technical Skills

* Networking: LAN/WAN, VLAN, Subnetting, Routing (Static/RIP), TCP/IP.
* DevOps & Cloud: Docker, Kubernetes, Git/GitHub, CI/CD Pipelines, AWS (EC2, VPC, IAM).
* Operating Systems: Linux Administration (Ubuntu/CentOS), Bash Scripting.
* Embedded Systems: C/Embedded C, ATmega32, Proteus Simulation, MATLAB/Simulink.

---

## Projects

* Real-Time Digital Clock: A high-precision 24-hour clock system using ATmega32 and LCD display with full Proteus simulation.
* Cloud-Native Patient Web Interface: Built a CI/CD pipeline, containerized a Node.js app, and deployed it on AWS using Terraform and Kubernetes.
* Traffic Light Control System: Designed and implemented traffic light logic using Embedded C and simulated the hardware in Proteus.
* Enterprise Network Design: Designed and simulated a company network using Cisco Packet Tracer with VLAN segmentation and routing.
* Linux & DevOps Practice: Automated system administration tasks using Bash scripts and managed containerized deployments.

---

## Education

* Bachelor of Science in Communications & Electronics Engineering
* Higher Technological Institute (HTI), 10th of Ramadan
* 2021 - 2026 | GPA: 3.41/4.0

---

## Contact & Links

* Portfolio: https://ebrahem-gamal.github.io/portfolio/#about
* LinkedIn: https://linkedin.com/in/ebrahem-gamal-network
* Email: ebrahemgamal0321@gmail.com
* GitHub: https://github.com/Ebrahem-Gamal

---
"Automating the world, one container at a time."
